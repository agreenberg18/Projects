 ...
 * and methods that control drawing parameters. The methods {@code
 * StdDraw.line()} and {@code StdDraw.point()} draw lines and points; the
 ...
 * and represented in degrees. For example, {@code StdDraw.arc(0.0, 0.0, 1.0, 0,
 * 90)} draws the arc of the unit circle from 3 o'clock (0 degrees) to 12
 * o'clock (90 degrees).
 * <p>
 ...
 * {@link #WHITE}, and {@link #YELLOW}. For example, {@code
 ...
 * called once, at the very beginning of a program. For example, {@code
 ...
 * specify a color of your choice. For example, {@code
 * StdDraw.clear(StdDraw.LIGHT_GRAY)} clears the canvas to a shade of gray. Most
 * often, these two methods are used in conjunction with animation mode.
 * <p>
 ...
 * @author Kevin Wayne
 */
public final class StdDraw2 implements ActionListener, MouseListener,
		MouseMotionListener, KeyListener {

	static int p = 1;
	static int q = MazeC2.giveN();
	char Flag = 'A';
	static boolean a = false;
	public static int[][] position = new int[22][22];
	...
	private static StdDraw2 std = new StdDraw2();
	...
	private static void init() {
		...
		frame.setTitle("Maze Game Player2");
		frame.pack();
	 ...
	 * <p>
	 * The predefined pen colors are <tt>StdDraw.BLACK</tt>,
	 * <tt>StdDraw.BLUE</tt>, <tt>StdDraw.CYAN</tt>, <tt>StdDraw.DARK_GRAY</tt>,
	 * <tt>StdDraw.GRAY</tt>, <tt>StdDraw.GREEN</tt>,
	 * <tt>StdDraw.LIGHT_GRAY</tt>, <tt>StdDraw.MAGENTA</tt>,
	 * <tt>StdDraw.ORANGE</tt>, <tt>StdDraw.PINK</tt>, <tt>StdDraw.RED</tt>,
	 * <tt>StdDraw.WHITE</tt>, and <tt>StdDraw.YELLOW</tt>.
	 * 
	 * @param color
	...
	/**
	 * Draws a line segment between (<em>x</em><sub>0</sub>, <em>y</em>
	 * <sub>0</sub>) and (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>).
	 * 
	 * @param x0
	...
	private static void pixel(double x, double y) {
		offscreen.fillRect((int) Math.round(scaleX(x)), (int) Math
				.round(scaleY(y)), 1, 1);
	}
	...
	private static Image getImage(String filename) {
		...
		if ((icon == null)
				|| (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
			URL url = StdDraw2.class.getResource(filename);
			if (url != null)
				icon = new ImageIcon(url);
		}
	 ...
	 * drawing methods such as {@code line()}, {@code circle()}, and {@code
	...
	public static void save(String filename) {
		...
		// reference:
		// http://archives.java.sun.com/cgi-bin/wa?A2=ind0404&L=java2d
		// -interest&D=0&P=2727
			...
			DirectColorModel cm = (DirectColorModel) onscreenImage
					.getColorModel();
			DirectColorModel newCM = new DirectColorModel(cm.getPixelSize(), cm
					.getRedMask(), cm.getGreenMask(), cm.getBlueMask());
	...
	public void actionPerformed(ActionEvent e) {
		FileDialog chooser = new FileDialog(StdDraw2.frame,
				"Use a .png or .jpg extension", FileDialog.SAVE);
		chooser.setVisible(true);
		String filename = chooser.getFile();
		if (filename != null) {
			StdDraw2.save(chooser.getDirectory() + File.separator
					+ chooser.getFile());
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = StdDraw2.userX(e.getX());
			mouseY = StdDraw2.userY(e.getY());
			mousePressed = true;
		}
	}
	...
	public void keyReleased(KeyEvent e) {
		if (Flag == 'D') {
			MazeC2.mazeprogramDown();
		} else if (Flag == 'U') {
			MazeC2.mazeprogramUp();
		} else if (Flag == 'R') {
			MazeC2.mazeprogramRight();
		} else if (Flag == 'L') {
			MazeC2.mazeprogramLeft();
		}
